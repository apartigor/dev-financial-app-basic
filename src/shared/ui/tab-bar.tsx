...
const tabDefs = [
  ...
  { id: "new",       href: "/debts/new", Icon: Plus,     key: "new" as const },
...
export function TabBar() {
  ...
  const labels: Record<string, string> = {
    home: t.nav.home ?? "", debts: t.nav.debts ?? "", new: t.nav.new ?? "", settings: t.nav.settings ?? "",
  }
        ...
        {tabDefs.map(({ id, href, Icon, key }) => {
          ...
          return (
            <Link key={id} href={href}
              className={cn("relative flex-1 flex flex-col items-center gap-0.5 py-1.5 transition-colors duration-200",
                active ? "text-ink" : "text-ink-faint")}>
              <span className={cn("relative flex items-center justify-center rounded-[10px] transition-all duration-200",
                isNew ? "bg-ink w-8 h-8" : "w-auto h-auto", active && !isNew && "scale-110")}>
                <Icon size={isNew ? 18 : 22} strokeWidth={isNew ? 2.2 : 1.75} className={isNew ? "text-bg" : ""} />
                <span className="absolute -top-1 -right-1"><LinkPendingIndicator /></span>
              </span>
