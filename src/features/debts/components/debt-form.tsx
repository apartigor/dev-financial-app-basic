...
export function DebtForm({ defaultValues, debtId }: DebtFormProps) {
  ...
  function addCustomReminder(date: string) {
    const current = form.getValues("customReminders")
    if (!current.includes(date)) {
      form.setValue("customReminders", [...current, date])
    }
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="pb-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <TextField
        label="Título"
        placeholder="Ex. Aluguel, Churrasco"
        {...form.register("title")}
      ...
      <div className="bg-surface border border-hairline rounded-md p-[18px] mb-4">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-sm font-medium">Me lembre</span>
        </div>
        <p className="text-[12px] text-ink-faint mb-3.5">
          Quando enviar notificação antes do vencimento?
        </p>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  active ? "bg-accent-soft border-transparent" : "bg-transparent border-hairline"
                )}
              >
                <span className={cn(
                  "w-5 h-5 rounded-[6px] border-[1.5px] flex items-center justify-center flex-shrink-0 transition-all",
                  active ? "bg-accent border-accent" : "bg-transparent border-hairline-strong"
                )}>
        ...
        <button
          ...
          className="mt-2.5 w-full border border-dashed border-hairline-strong rounded-sm py-2.5 px-3
            flex items-center gap-2 text-[13px] text-ink-muted cursor-pointer bg-transparent"
        >
          <Plus size={14} /> Adicionar data específica
        </button>

        {customPickerOpen && <MiniCalendar dueDate={dueDate} onPick={addCustomReminder} />}
      </div>
      ...
      <div className="bg-surface border border-hairline rounded-md p-[18px] mb-4">
        ...
        <p className="text-[12px] text-ink-faint mb-3.5">
          Opcional. Divida o valor total entre várias pessoas.
        </p>
        ...
        {hasSplits && totalCents > 0 && (
          <div className={cn(
            ...
            <span className="font-medium">
              {remaining === 0 && !hasZeroItem && "✓ Valor totalmente dividido"}
              {remaining === 0 && hasZeroItem && "⚠ Alguma cobrança está com valor R$ 0"}
              {remaining > 0 && `Falta dividir ${formatBRL(remaining)}`}
              {remaining < 0 && `⚠ Ultrapassou em ${formatBRL(-remaining)}`}
            </span>
        ...
        >
          <Plus size={14} />
          {hasSplits ? "Adicionar outra pessoa" : "Adicionar pessoa"}
        </button>
      </div>
      ...
      <div className="mb-6">
        <Checkbox
          checked={form.watch("recurrenceRule") !== "none"}
          onChange={() =>
            form.setValue(
              "recurrenceRule",
              form.getValues("recurrenceRule") === "none" ? "monthly" : "none"
            )
          }
          label="Esta dívida se repete todo mês"
        />
      </div>
      ...
      <div className="flex gap-2.5">
        ...
        <Button type="submit" disabled={loading || splitInvalid} style={{ flex: 2 }}
          title={splitInvalid ? "Ajuste o valor das cobranças para corresponder ao total" : undefined}>
          {loading ? "Salvando…" : splitInvalid ? "Ajuste as cobranças" : "Salvar dívida"}
        </Button>
      </div>
    </form>
  )
